 ...
 * @version 1.0
 * */
public class Tab2RightFragment extends BasePageFragment implements
		OnItemClickListener {
	...
	public static Fragment newInstance() {
		Tab2RightFragment fragment = new Tab2RightFragment();
		return fragment;
	}
	...
	protected void reqRemoteDatas(boolean _isForceFetchData) {
		LogUtil.d(Tab2RightFragment.this.getClass().getSimpleName(),"加载数据……reqRemoteDatas");
		leftqryData();
	}
	...
	private void leftqryData() {
		...
		JSONObject jsonObject=new JSONObject();
		try {
			 jsonObject.put("subCategoryId", SubCategoryBean.ycqId);
			 String encodeUrl=URLEncoder.encode(jsonObject.toString(),"UTF-8");
		...
		public void onSuccess(int code, int arg1, int arg2, Object obj) {
			switch (code) {
				case RequestTaskIds.LeftFragment_InfoType_Qry:
					...
					LogUtil.d(Tab2RightFragment.this.getClass().getSimpleName(),obj.toString());
