 ...
 * @version 1.0
 * */
public class Tab2LeftFragment extends BasePageFragment implements
		OnItemClickListener {
	...
	public static Fragment newInstance() {
		Tab2LeftFragment fragment = new Tab2LeftFragment();
		return fragment;
	}
	...
	protected void reqRemoteDatas(boolean _isForceFetchData) {
		LogUtil.d(Tab2LeftFragment.this.getClass().getSimpleName(),"加载数据……reqRemoteDatas");
		leftqryData();
	}
	...
	private void init() {
		...
		leftAdapter = new CommonAdapter<InfoType>(context,R.layout.tab2_frag_left_item,
			...
			@Override
			public void convert(CommonAdapter.ViewHolder helper,
	...
	private void leftqryData() {
		...
		JSONObject jsonObject=new JSONObject();
		try {
			 jsonObject.put("subCategoryId", SubCategoryBean.aqqId);
			 String encodeUrl=URLEncoder.encode(jsonObject.toString(),"UTF-8");
		...
		public void onSuccess(int code, int arg1, int arg2, Object obj) {
			switch (code) {
				case RequestTaskIds.LeftFragment_InfoType_Qry:
					...
					LogUtil.d(Tab2LeftFragment.this.getClass().getSimpleName(),obj.toString());
