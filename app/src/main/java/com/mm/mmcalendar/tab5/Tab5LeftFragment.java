 ...
 * @version 1.0
 * */
public class Tab5LeftFragment extends BasePageFragment implements
		OnItemClickListener {
	...
	public static Fragment newInstance() {
		Tab5LeftFragment fragment = new Tab5LeftFragment();
		return fragment;
	}
	...
	protected void reqRemoteDatas(boolean _isForceFetchData) {
		LogUtil.d(Tab5LeftFragment.this.getClass().getSimpleName(),"加载数据……reqRemoteDatas");
		leftqryData();
	}
	...
	private void leftqryData() {
		...
		JSONObject jsonObject=new JSONObject();
		try {
			 jsonObject.put("subCategoryId", SubCategoryBean.yedyId);
			 String encodeUrl=URLEncoder.encode(jsonObject.toString(),"UTF-8");
		...
		public void onSuccess(int code, int arg1, int arg2, Object obj) {
			switch (code) {
				case RequestTaskIds.LeftFragment_InfoType_Qry:
					...
					LogUtil.d(Tab5LeftFragment.this.getClass().getSimpleName(),obj.toString());
