...
public class CrashHandler implements UncaughtExceptionHandler {
	...
	private static final String FILE_NAME = "crashlog" + CRASH_REPORTER_EXTENSION;
	...
	private boolean handleException(Throwable ex) {
		...
		new Thread() {
			...
			public void run() {
				Looper.prepare();
				Toast.makeText(mContext, "程序异常:" + msg, Toast.LENGTH_LONG).show();
				Looper.loop();
			}
	...
	private String saveCrashInfoToFile(Throwable ex) {
		...
		if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
			try {
				...
				String time = SimpleDateFormat.getInstance().format(new Date()) + "\n";
	...
	public void collectCrashDeviceInfo(Context context) {
		try {
			...
			if (pi != null) {
				deviceInfo.put("versionName", pi.versionName == null ? "null" : pi.versionName);
