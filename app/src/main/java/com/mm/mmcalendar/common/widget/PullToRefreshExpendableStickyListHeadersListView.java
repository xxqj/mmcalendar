...
public class PullToRefreshExpendableStickyListHeadersListView extends PullToRefreshBase<ExpandableStickyListHeadersListView> {

    private static final OnRefreshListener<ExpandableStickyListHeadersListView> defaultOnRefreshListener = new OnRefreshListener<ExpandableStickyListHeadersListView>() {

        @Override
        public void onRefresh(PullToRefreshBase<ExpandableStickyListHeadersListView> refreshView) {

        }

    };

    public PullToRefreshExpendableStickyListHeadersListView(Context context) {
        super(context);
    ...
    public PullToRefreshExpendableStickyListHeadersListView(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public PullToRefreshExpendableStickyListHeadersListView(Context context, Mode mode) {
        super(context, mode);
    ...
    public PullToRefreshExpendableStickyListHeadersListView(Context context, Mode mode, AnimationStyle style) {
    ...
    protected ExpandableStickyListHeadersListView createRefreshableView(Context context, AttributeSet attrs) {
        ExpandableStickyListHeadersListView StickySwipe;

        if (VERSION.SDK_INT >= VERSION_CODES.GINGERBREAD) {
            StickySwipe = new InternalStaggeredGridViewSDK9(context, attrs);
        } else {
            StickySwipe = new ExpandableStickyListHeadersListView(context, attrs);
        }
...
//        int itemCount = getRefreshableView().getAdapter().getCount();
        int itemCount = getRefreshableView().getCount();
    ...
    @TargetApi(9)
    final class InternalStaggeredGridViewSDK9 extends ExpandableStickyListHeadersListView {
        ...
        protected boolean overScrollBy(int deltaX, int deltaY, int scrollX, int scrollY, int scrollRangeX,
            ...
            // Does all of the hard work...
            OverscrollHelper.overScrollBy(PullToRefreshExpendableStickyListHeadersListView.this, deltaX, scrollX, deltaY,
